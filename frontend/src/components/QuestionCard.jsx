...
const QuestionCard = ({ question, onToggleLike, onToggleBookmark }) => {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: "Check out this question", url: shareUrl }).catch(console.error);
    } else {
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl border border-gray-200 max-w-2xl mx-auto mb-6 overflow-hidden transition hover:shadow-xl">
      <div className="p-6 sm:p-8 flex flex-col gap-4">
        {/* Question Text */}
        <div
          className="text-gray-900 font-medium text-sm sm:text-base leading-relaxed break-words"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="flex items-center gap-2 text-2xl text-gray-500 hover:text-blue-500 transition"
            >
              <MessageCircle size={24} />
              <span className="text-base">{question.commentsCount || 0}</span>
            </button>

            {/* Share */}
            <button
              onClick={handleShare}
              className="flex items-center text-2xl text-gray-500 hover:text-green-500 transition"
            >
              <Share size={24} />
            </button>
          </div>
